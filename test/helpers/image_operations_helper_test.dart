...
void main() {
  group('ImageOperationsHelper', () {
    ...
    test('renameAllFiles shuffles captions when some images lack captions',
      ...
      await File(p.join(tempDir.path, '03_image.txt'))
      ...
      expect(fileNames, containsAll(['01.jpg', '02.jpg', '03.jpg']));

      // Check that captions are correctly named
      expect(fileNames, contains('01.txt'));
      expect(fileNames, isNot(contains('02.txt')),
          reason: '02.txt should not be created as 02_image.jpg has no caption.');
      expect(fileNames, contains('03.txt'),
          reason:
              '03.txt should be created for 03_image.jpg with its caption.');

      // Check the content of the captions
      final caption1Content =
          await File(p.join(tempDir.path, '01.txt')).readAsString();
      expect(caption1Content, 'caption for 01');

      final caption3Content =
